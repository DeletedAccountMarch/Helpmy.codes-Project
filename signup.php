...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    $existquery =  protect("select * from account where email ='$email'");
    ...
    if($row > 0){
        ...
        if(($password == $cpassword)){
            ...
            $v_hash = md5($email);
            $status=0;
            $sql = "INSERT INTO account (username,email,passwords,v_token,v_status) VALUES ('$name','$email','$hash','$v_hash','$status')";
